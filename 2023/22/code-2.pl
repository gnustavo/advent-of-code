...
sub disintegrate {
    ...
    my $sustaineds = $brick->[2];
    my @sustaineds_disintegrated;
    my @sustaineds_kept;
    foreach my $sustained (@$sustaineds) {
        if ($sustained->[3] == 1) {
            push @sustaineds_disintegrated, $sustained;
        } else {
            push @sustaineds_kept, $sustained;
        }
        $sustained->[3] -= 1;
    }
    local $brick->[2] = \@sustaineds_kept;

    my $nof_disintegrated = 0;
    foreach (@sustaineds_disintegrated) {
        $nof_disintegrated += disintegrate($_);
    }

    $_->[3] += 1 foreach @$sustaineds;

    return $nof_disintegrated;
}

my $nof_disintegrated = 0;
foreach my $brick (@fallen) {
    $nof_disintegrated += disintegrate($brick);
}
say $nof_disintegrated;
